// HaralickDirectionalityRot.cpp : Defines the entry point for the console application.
//
			...
			for (int x = ProcOptions.offsetTileX; x <= (maxX - ProcOptions.offsetTileX); x += ProcOptions.shiftTileX)
			{
				...
				for (int offset = ProcOptions.minOfset; offset <= ProcOptions.maxOfset; offset += 1)
				{
					for (int angleIndex = 0; angleIndex < stepNr; angleIndex++)
					{
						...
						Mat SmallImRoratated, RoiRotated;

						Point rotationCenter = Point(SmallIm.cols / 2, SmallIm.rows / 2);
						Mat rotationMatrix = getRotationMatrix2D(rotationCenter, angle, 1);
						warpAffine(SmallIm, SmallImRoratated, rotationMatrix, SmallIm.size());
						warpAffine(Roi, RoiRotated, rotationMatrix, Roi.size());

						Mat SmallImToShow = ShowImageF32PseudoColor(SmallImRoratated, minNorm, maxNorm);
						imshow("ImageSmallRotated", ShowSolidRegionOnImage(GetContour5(RoiRotated), SmallImToShow));
						waitKey(100);


						Mat COM;
						
						COM = COMHorizontalRoi(SmallIm, Roi, offset, ProcOptions.binCount, maxNorm, minNorm, 1);
